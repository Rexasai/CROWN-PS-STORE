...
app.post("/api/gacha/roll", authMiddleware, async (req, res) => {
  ...
  const cost = state.settings.gachaCost;
  ...
  await db.write((d) => {
    ...
    u.xp = (u.xp || 0) + 25;
    addInv(u, { name: r.name, rarity: r.rarity, icon: r.icon, qty: 1 });
    u.gachaHistory.push({ ...r, time: new Date().toISOString() });
    result = { reward: r, tickets: u.tickets, xp: u.xp, level: Math.floor(u.xp / 100) + 1 };
  });
